...
class SlidingWindowPredictor(BaseModel):
    ...
    def predict_sliding_windows(
        self, video_path: str, model_predict_fn, preprocess_fn, return_per_frame: bool = True
    ) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            all_frames = load_full_video_frames(
                video_path=video_path, target_size=(224, 224), target_fps=self.target_fps
            )
    ...
    def _create_predictive_frame_array(
        self, target_frames: List[int], predictions: List[float], total_frames: int
    ) -> np.ndarray:
        """
