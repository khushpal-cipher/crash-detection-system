...
def load_badas_model(
    ...
    if checkpoint_path is None and download_weights:
        ...
        checkpoint_path = hf_hub_download(
            repo_id="nexar-ai/badas-open", filename="weights/badas_open.pth"
        )
...
class BADASModel:
    ...
    def estimate_time_to_accident(
        self, collision_probs: List[float], fps: float = 8.0
    ) -> Optional[float]:
        """
...
def preprocess_video(
    video_path: str, target_fps: float = 8.0, num_frames: int = 16, img_size: int = 224
) -> torch.Tensor:
    """
