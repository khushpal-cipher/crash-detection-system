...
Examples:
  ...
  badas-predict video.mp4 --device cuda --format detailed
    ...
    parser.add_argument(
        "--device", type=str, default=None, help="Device to use: cuda, cpu, or auto (default: auto)"
    )
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Path to custom model checkpoint"
    )
    ...
    try:
        model = BADASModel(
            device=args.device, confidence_threshold=args.threshold, checkpoint_path=args.checkpoint
        )
        ...
        with open(output_path, "w") as f:
            json.dump(
                (
                    results
                    if args.format == "json"
                    else {"predictions": predictions, "threshold": args.threshold}
                ),
                f,
                indent=2,
            )
...
def format_results(predictions, threshold, format_type):
    ...
    if format_type == "json":
        high_risk = [(i * 0.125, float(p)) for i, p in enumerate(predictions) if p >= threshold]
        return {
        ...
        high_risk = [(i * 0.125, p) for i, p in enumerate(predictions) if p >= threshold]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="badas", description="BADAS: Advanced Driver Assistance System CLI"
    )
    ...
    # Predict command
    predict_parser = subparsers.add_parser("predict", help="Predict collision risk in video")
